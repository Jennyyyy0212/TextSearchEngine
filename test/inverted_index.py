...
def process_text(text):
    ...
    filtered_words = [stemmer.stem(token) for token in tokens if token not in stopwords]
...
def build_inverted_output(input_dir, output_dir,temp_dir = "temp_unzip_dir"):
    if os.listdir(output_dir): #avoid run huge data 
        print(f"{output_dir} already have inverted index.")
        return
